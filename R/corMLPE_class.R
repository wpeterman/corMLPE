...
#' @return `corMLPE()` returns an object of class `corMLPE` and `corStruct`
#'   for use as a correlation structure in `nlme::gls()` or `nlme::lme()`.
#'   The object stores the MLPE correlation parameter and the formula that
#'   identifies the two members of each pairwise observation. The
#'   `as.matrix()` method returns the implied observation-level correlation
#'   matrix for an initialized `corMLPE` object.
#' @examples
...
#' @return The `t()` method returns a transposed `dgCMatrix_corMLPE` object.
#'   The `[` methods return row- or column-subsets while preserving the
#'   `dgCMatrix_corMLPE` class. These methods are used internally so sparse
#'   matrices generated by `corMLPE` remain compatible with downstream model
#'   fitting code.
#' @examples
